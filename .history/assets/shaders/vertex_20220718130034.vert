...
void main(){
    ...
    vec3 cc = palette(aNormal.y, a, b, c, d);
    ...
    float diplacement2 = u_amp * easeInSine(u_amp) * pnoise(aNormal + u_time * 0.01, vec3(0.0));
    // newPosition.z += displacement * aNormal.z * amplitude;
    vNoise = pnoise(aNormal + u_time * 0.01, cc);
    vec4 newPosition = vec4(aPosition + easeOutSine(diplacement2) * aNormal, 1.0);
    vNormal = aNormal;
    time = u_time;
